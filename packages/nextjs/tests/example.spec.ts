...
test.describe("ABI Ninja", () => {
  ...
  test.describe("Cross-chain Contract Interactions", () => {
    ...
    test("should load an unverified contract on Sepolia and decompile successfully using heimdall backend", async ({
      ...
      await page.getByRole("textbox", { name: "Contract address" }).fill("0x759c0e9d7858566df8ab751026bedce462ff42df");

      // Wait for decompile button to be ready and click it
      const decompileButton = page.getByRole("button", { name: "Decompile (beta)" }).nth(2);
      await decompileButton.waitFor({ state: "visible", timeout: 30000 });
      await page.waitForTimeout(1000); // Allow button to become active
      await decompileButton.click();

      // Wait for decompilation to complete
      await expect(page.getByRole("button", { name: "changeOwner" })).toBeVisible({ timeout: 90000 });
    });
